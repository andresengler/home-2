...
import { geistSans, azeretMono } from '../fonts'
...
const renderSection = (section: any) => (
  ...
  >
    <h2 className={`${azeretMono.className} text-[12px] font-normal tracking-tight text-gray-400 dark:text-gray-500`}>
      {section.title}
    </h2>
    ...
    {section.title === 'Languages' ? (
      <div className="flex items-center">
        {section.items.map((item: any, index: number) => (
          ...
          >
            <div>
              <h3 className={`${geistSans.className} text-[15px] font-medium text-gray-800 dark:text-white`}>
                {item.spanish.company}
              </h3>
              <p className={`${geistSans.className} text-[15px] text-gray-600 dark:text-gray-300 leading-relaxed`}>
                {item.spanish.role}
              </p>
            </div>
            <div>
        ...
        {section.items.map((item: any, index: number) => (
          ...
          >
            <Link href={item.href} className="group block">
              <motion.p 
                className={`${geistSans.className} text-[15px] leading-snug`}
                whileHover={{ x: 2 }}
              ...
              >
                ...
                <span className={`${azeretMono.className} text-gray-400 dark:text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity text-[0.7em] ml-1`}>
                  ↗
                </span>
              </motion.p>
            </Link>
        ...
        {section.items.map((item: any, index: number) => (
          ...
          >
            <div className="space-y-1">
              {item.date && (
                <p className={`${azeretMono.className} text-[12px] text-[#C5B6B6] dark:text-[#C5B6B6]`}>
                  {item.date}
                </p>
              )}
              <h3 className={`${geistSans.className} text-[15px] font-medium text-gray-800 dark:text-white`}>
...
export default function Resume() {
  ...
  return (
    <div className={`${geistSans.className} min-h-screen bg-white dark:bg-black text-gray-800 dark:text-white relative pb-2 transition-colors duration-200`}> 
      <div className="space-y-10">
        <div> 
          <h2 className={`${azeretMono.className} text-[12px] font-normal tracking-tight text-gray-400 dark:text-gray-500`}>
            Resume
          </h2>
          <p className={`${geistSans.className} text-[15px] text-gray-600 dark:text-gray-300 leading-relaxed mt-4`}>
            ...
            {resumeData.map((section) => (
              <li key={section.title}>
                <button
                  ...
                  className={`${azeretMono.className} text-[12px] font-normal tracking-tight whitespace-nowrap transition-colors ${
